<template>
  <div>
    <div
      class="sphereItem"
      :style="`top: ${get_top()}em; left: ${get_left()}em; filter: ${get_blur()}; 
        opacity: ${get_opacity()}; font-family: Arial;
        position: absolute;
        transition: all 0.0s;
        white-space: nowrap;
        display: flex;
        ...
        align-items: center;`"
    >
      <img :src="item.image" :style="`width: ${width}px`" />
      <h1
        class="transition-colors duration-300 ease-linear dark:text-white"
        :style="`font-size: ${get_size()}rem;`"
      >
        {{ item.name }}
      </h1>
    </div>
...
export default {
  ...
  methods: {
    ...
    rotate_pos() {
      ...
      var z = this.pos_z;
      this.pos_x =
        this.rotation_matrix[0][0] * x +
        this.rotation_matrix[0][1] * y +
        this.rotation_matrix[0][2] * z;
      this.pos_y =
        this.rotation_matrix[1][0] * x +
        this.rotation_matrix[1][1] * y +
        this.rotation_matrix[1][2] * z;
      this.pos_z =
        this.rotation_matrix[2][0] * x +
        this.rotation_matrix[2][1] * y +
        this.rotation_matrix[2][2] * z;
    ...
    set_starting_points() {
      ...
      this.angle_phi = this.to_degrees(
        (2 * Math.PI * this.index) / goldenRatio
      );
      this.angle_theta = this.to_degrees(
        Math.acos(1 - (2 * (this.index + 0.5)) / this.number_items)
      );
