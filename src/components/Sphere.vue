<template>
    <div>
        <div class="wordSphere" id="sphere" @mouseenter="start_roll" @mousemove="update_roll" @mouseleave="end_roll"
            :style="`width: ${get_area_width()}rem; height: ${get_area_height()}rem; position: relative`">
            <SphereItem
...
export default {
    ...
    data() {
        return {
            ...
            speed: 0,
            slowing_factor: 100,  // change this one at will
    ...
    methods:{
        ...
        calc_padding(){
            ...
            var max = this.items_list[0].name;
            for (let i = 1; i < this.items_list.length; i++){
                if (max.length < this.items_list[i].name.length){
                    max = this.items_list[i].name;
                }
            }
